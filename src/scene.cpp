...
void Scene::addSphere(Sphere &sphere) {
    GPUSphere gpuSphere{.center = glm::vec3(sphere.center.x, sphere.center.y, sphere.center.z),
...
Scene buildThreeSpheres() {
    ...
    scene.updateLight({-0.15, 0.49, -0.15}, {0.3, 0.0, 0.0}, {0.0, 0.0, 0.3});
...
Scene buildCube() {
    ...
    int base = 0;
    scene.addIndices({base + 0, base + 1, base + 2, base + 0, base + 2, base + 3,
...
Scene buildObj() {
    ...
    scene.updateLight({-2.5, 2.49, 4.5}, {0.5, 0.0, 0.0}, {0.0, 0.0, 0.5});

    return scene;
}

std::array<int, 36> getCubeIndices(int base) {
    return {base + 0, base + 1, base + 2, base + 0, base + 2, base + 3, base + 4, base + 6,
            base + 5, base + 4, base + 7, base + 6, base + 4, base + 0, base + 3, base + 4,
            base + 3, base + 7, base + 1, base + 5, base + 6, base + 1, base + 6, base + 2,
            base + 4, base + 5, base + 1, base + 4, base + 1, base + 0, base + 3, base + 2,
            base + 6, base + 3, base + 6, base + 7};
}
...
Mesh createCubeMesh(Scene &scene, const Vec3 &corner, const Vec3 &dim, int matId) {
    ...
    scene.addVertices({{corner + Vec3{0, 0, 0}},
...
void rotateMesh(Scene &scene, Mesh &mesh, float angle) {
    ...
    for (int i = vertexOffset; i < vertexOffset + vertexCount; i++) {
        ...
        glm::vec3 rotated = glm::rotate(pos, glm::radians(angle), glm::vec3(0.0f, 1.0f, 0.0f));

        scene.vertices[i].position = rotated;

        scene.cpuVertices[i].position = {rotated.x, rotated.y, rotated.z};
    }
}
